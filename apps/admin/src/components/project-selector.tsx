					...
					{projects.map((p) => (
						<button
							...
							className={`w-full text-left px-3 py-2 text-sm hover:bg-zinc-800 transition-colors ${
								p.id === currentProject?.id ? 'bg-zinc-800 text-white' : 'text-zinc-400'
							}`}
						>
						...
						{creating ? (
							<div className="p-2 flex gap-1">
								<input
									...
									className="flex-1 px-2 py-1 bg-zinc-800 border border-zinc-700 rounded text-xs focus:outline-none"
									autoFocus
								/>
								<button
									type="button"
							...
							<button
								...
								className="w-full text-left px-3 py-2 text-sm text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300"
							>
								+ New Project
							</button>
						)}

...
export async function scimRoutes(app: FastifyInstance) {
	async function authenticateScim(request: FastifyRequest, reply: FastifyReply) {
		...
		const [tokenRow] = await app.db
			...
			.where(
				and(
					eq(scimTokens.connectionId, connection.id),
		...
		await app.db
			...
			.where(eq(scimTokens.id, tokenRow.id))
	...
	app.get<{
		Params: { slug: string }
		Querystring: { filter?: string; count?: string; startIndex?: string }
	}>('/:slug/v2/Users', { preHandler }, async (request) => {
		const connection = (
			request as unknown as { scimConnection: typeof ssoConnections.$inferSelect }
		).scimConnection
		...
		async (request, reply) => {
			...
			if (!userName || !externalId) {
				return reply.status(400).send({
					schemas: ['urn:ietf:params:scim:api:messages:2.0:Error'],
					detail: 'userName and externalId required',
					status: '400',
				})
			}
			...
			const [identity] = await app.db
				...
				.where(
					and(
						...
						eq(userIdentities.subject, externalId),
					),
				)
				.limit(1)
			if (!identity) {
	...
	}>('/:slug/v2/Users/:id', { preHandler }, async (request, reply) => {
		const connection = (
			request as unknown as { scimConnection: typeof ssoConnections.$inferSelect }
		).scimConnection
	...
	app.get<{ Params: { connectionId: string } }>(
		...
		async (request, reply) => {
			const [conn] = await app.db
				...
				.where(
					and(
						eq(ssoConnections.id, request.params.connectionId),
